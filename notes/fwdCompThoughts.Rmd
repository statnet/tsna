...
```{r}
as.tPath.transmat<-function(x,n,seed,...){
  if(missing(n)){
    # we don't know the actuall size of the network, so use max id
    n<-max(x$sus,x$inf)
  }
  if(missing(seed)){
    # if no infections have occured because seed vertex is an isolate
    # the transmat will be empty, so the seed argument is a way to pass that in
    # otherwise, assume it is the first infector vertex
    seed<-x$inf[1]
  }
  tdist<-rep(Inf,n)
  previous<-rep(0,n)
  ...
  out<-list(tdist=tdist,previous=previous,gsteps=gsteps,start=start,end=end,directed='fwd',type='EpiModel.transmat')
...
epiPath<-function(nD,v, inf.prob){
  ...
  control <- control.net(type = "SI", start=tr[1], nsteps = tr[2], nsims = 1,
                       ...
                       skip.check = TRUE, save.nwstats = FALSE, save.network = FALSE,save.transmat = TRUE)
  episim<-netsim(nD,param,init,control)
  tp<-as.tPath.transmat(get_transmat(episim),n=network.size(nD),seed=v)
  return(tp)
}
```
...
generationTime<-function(tp){
  include<-which(tp$previous>0 & tp$tdist<Inf)
...
In the case that the edges of a network are always active, the algorithm should reduce to the shortest path problem, does it?
...
Easy to compute a tdist matrix of paths from each vertex to every other. Computing a full betweeness would require also knowing the number of paths passing through each vertex. Can compute this by tabulating the number of times each vertex appears as 'previous', however this is not consistant with the standard defintions of betweeness centrality which require know the number of paths of length equal to the shortest path. 

The static definition of betweenness centrality for vertex n is computed as the sum for all vertices (not including n) the number of shortest paths from s to t that n is on, divided by the the number of shortest paths from s to t.  I assume that 'number of shortest paths' means number of paths that are the same length as the shortest path?  This normalization to account for equal alternate paths can't becomputed by this implementation of the algorithm because it uses depth first search and does not tabulate the alternate equally short journeys.   

We can compute something like stress centrality but just counting the number of earliest paths that each vertex lies on, but I doubt that this is an useful measure, since ideally we'd do some sort of summation across all of the indirect paths, not just shortest. 
...
Compute for a network, and compare with mean of static betweeness, and with betweeness calcualted in the time projected network

```{r}
dynBet<-dynStressCent(short.stergm.sim)
meanBet<-colMeans(tSnaStats(short.stergm.sim,'betweenness')[1:24,])
proj<-timeProjectedNetwork(short.stergm.sim)
meanProjBet<-aggregate(data.frame(stresscent(proj),network.vertex.names(proj)),list( network.vertex.names(proj)),mean)[,2]
plot(dynBet,meanProjBet,main='dynamic stress centrality vs \nmean stress cent of each vertex in time projected net')
text(dynBet,meanProjBet,network.vertex.names(short.stergm.sim),cex=0.5, col='blue',pos=4)
plot(dynBet,meanBet,main='dynamic stress centrality vs\n mean of momentary stress cent at each time point')
text(dynBet,meanBet,network.vertex.names(short.stergm.sim),cex=0.5, col='blue',pos=4)
```


however, should be possible to calculate a some closeness centrality, using the temporal distances returned, we could call it 'soonness' .  This would be a relative measure of how early in the network's evolution a vertex is able to reach other vertices.  
...
gives a value of 1 for vertices that can never reach anyone, up to Inf (divide by zero) for vertices that can reach everyone in the first step.  Can prevent the divide by zero by using graph.step.time > 0 ?


```{r}
t1<-network.extract(short.stergm.sim,at=1)
dynStt1<- dynStressCent(t1,graph.step.time = 0.1)
strt1<- stresscent(as.sociomatrix(t1),gmode = 'graph')
plot(dynStt1,strt1)
```
...
```{r}
batchEpi<-function(networks,net.names,inf.probs, seed.count, times, sameSeeds=TRUE){
  ...
  sample.names<-rep(net.names,each=length(inf.probs)*seed.count*times)
  inf.probs<-sample(inf.probs,reps,replace = TRUE)
  if (sameSeeds){
    # draw the same seeds for each network, so we can compare inf.probs from same seed
    seedForNet<-lapply(1:nNets,function(n){
      seedSample<-sample(1:network.size(networks[[n]]),seed.count)
      return(seedSample)
    })
    seeds<-sapply(sample.names,function(nm){
      sample(seedForNet[[which(net.names==nm)]],1)
    })
  } else {
    # draw a random seed vertex for each network
...
treeStats<-data.frame(treeRuns$net.names,
           ...
           seed=sapply(1:runs,function(r){
             s<-which(treeRuns[[r]]$tdist==0)
             #hack, incase old code which didn't record seed for isolates
             #if(length(s)==0) {s<-0}
             s
           })
)
