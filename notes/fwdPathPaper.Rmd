...
The image below shows a projection of the seven time steps of the trivial network and highlights in red some possible forward temporal paths originating from vertex A at the first time step. 
...
```{r, echo=FALSE}
aggregationBias<-function(nd){
  timeRespecting<-tReach(nd)
  timeIgnoring<-tReach(as.networkDynamic(network.collapse(nd,rule='earliest')),
                       start=0,graph.step.time=1)
  return(1-sum(timeRespecting)/sum(timeIgnoring))
}
minimal<-network.initialize(5)
add.edges.active(minimal,tail=1:4,head=2:5,onset = c(1,3,1,3),terminus=c(2,4,2,4))
aggregationBias(minimal)

minimalGroup<-network.initialize(200)
add.edges(minimalGroup,1:network.size(minimalGroup),c(2:network.size(minimalGroup),1))
activate.edges(minimalGroup,e=seq(from=1,to=network.size(minimalGroup), by=2),onset = 2,terminus=3)
activate.edges(minimalGroup,e=seq(from=2,to=network.size(minimalGroup), by=2),onset = 0,terminus=2)

#plot(minimalGroup)
#aggregationBias(minimalGroup)
```
...
```{r,echo=FALSE}
# "the aver of the clock-time needed for an actor ro reach others in the network"
fastnessCentrality<-function(nd,graph.step.time=0){
    # compute the earliest fwd path from each vertx
  paths<-lapply(1:network.size(nd),function(v){
    tPath(nd,v,graph.step.time = graph.step.time)
    })
    bounds<-tsna:::get_bounds(nd)
  repVal<-bounds[2]-bounds[1]
  fast<-sapply(1:length(paths),function(v){
    tdist<-paths[[v]]$tdist
    # remove the v'th entry from the list
    tdist<-tdist[-v]
    # replace Infs with full duration for unreachable vertices
  ...
  return(1/(network.size(nd)-1)*fast)
}

toyFastCent<-fastnessCentrality(toy_epi_sim,graph.step.time = 1)
plot(toy_epi_sim,
     coord=coords,
     vertex.cex=sqrt(toyFastCent+1)/5,
     displaylabels=TRUE,
     edge.col='gray',
     label.cex=0.5,
     main='Aggregate toy_epi_sim network with vertex area\nproportional to fastness centrality' ) 
...
```{r,echo=FALSE}
toyBkwd<-tReach(toy_epi_sim,direction='bkwd')
plot(toy_epi_sim,coord=coords,vertex.cex=sqrt(toyBkwd+1)/10,
     displaylabels=TRUE,
     edge.col='gray',
     label.cex=0.5,
     main='Aggregate toy_epi_sim network with vertex area\nproportional to backwards component size' )
```
...
plot(base1v1.0[,2],baseStress,main='dynamic stress vs inf comp size')

baseEarly<-tiedTimes(base,v=baseSeedSet)
plot(base1v1.0[,2],100-baseEarly,main='earlist tie time vs inf comp size')

baseFast<-fastnessCentrality(base,graph.step.time=1)[baseSeedSet]
plot(base1v1.0[,2],baseFast,main='fastnessCentrality vs inf comp size')
```
