...
# check with counts
expect_equal(tEdgeDuration(moodyContactSim,mode='counts'),rep(1,18))
...
# spell level
expect_equal(tEdgeDuration(test,subject='spells'),c(2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1, 2, 1))
# edge level
expect_equal(tEdgeDuration(test,subject='edges'),rep(3,network.edgecount(test)))
expect_equal(tEdgeDuration(test,subject='dyads'),rep(3,network.edgecount(test)))
...
add.edges(test,1,2)
add.edges(test,1,2)
expect_equal(tEdgeDuration(test,subject='edges',start=0,end=2),c(2,2,2))
expect_equal(tEdgeDuration(test,subject='dyads',start=0,end=2),6)
# test counts
expect_equal(tEdgeDuration(test,mode='count',subject='edges',start=0,end=2),c(1,1,1))
