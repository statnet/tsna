...
Most `tsna` functions accept as their input continuous- and discrete-time longitudinal networks having vertex, edge, and attribute dynamics stored in the `networkDynamic` format.  `tsna` includes tools for applying "traditional" static Social Network Analysis (SNA) metrics at multiple time points, as well as temporal extensions of SNA metrics using forwards- and backwards-path routines. The initial version of `tsna` is primarily focused on working with discrete time networks of the type that might be generated from simulation. 

Research on formal methods for analysis of logitundinal networks is a realitively new area of study. Many of the measures included in this package may be formally identical to measures already published with different names in literature of diverse research fields.   We will attempt to find and cite such previous work, but if you encounter any omissions, please let us know. 
...
Depending on network topology, there may be lots of possible temporal paths between a single pair of vertices. The *earliest arriving temporal path* from vertex `i` to vertex `j` can be thought of as the soonest possible time a message sent from `i` to `j` could possibly arrive.  It is analogous to the shortest geodesic path in a static network and can be calculated with similar efficiency. 
...
Contrast this with the `monog` network has Bernoilli (0,1) cross-section degree distribution, meaning nobody has more than one partner at a time. This greatly reduces the spreading potential of the network, so almost all of the FRS are quite small. 
...
The `tEdgeDensity` function is a network-level metric to compute the *temporal density* of a networkDynamic object.  It can be thought of as a crude measure of the total connectivity in the network.  It is operationalized as a measure of the total amount of time vertices are connected by active edges in the network, divided by the amount of time they could possibly be connected.  

For all the dyads which are ever observed to have edges, what fraction of the time are they connected?
...
For all the possible dyads in the network, what fraction of time are they actually connected?
...
```{r}
connectedProb<-function(nd,trials=1000){
  n<-network.size(nd)
  i<-sample(1:n,size = trials,replace=TRUE)
  j<-sample(1:n,size = trials,replace=TRUE)
  trange<-range(get.change.times(nd))
  times<-sample(trange[1]:trange[2],size=trials,replace=TRUE)
  sapply(1:trials,function(c){
    length(get.edges.active(nd,v=i[c],alter=j[c],at=times[c]))
  })
}
```
...
This package is far from complete.  Most functions do not yet include ability to bin times necessary for working with certain types of continuous time networks.  Include vertex activity functions, and correctly account for vertex activity in edge funcitons. 
