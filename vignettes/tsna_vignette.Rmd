...
This vignette also makes use of various example datasets provided by the `networkDynamicData` package, and various static Social Network Analysis measures provided by the \code{\link{sna}} package. 
...
We would expect that since the concurrency sims all had the same dissolution paramter, they should all have similarly shaped distrubtions of edge durations
...
```{r}
which(tEdgeDuration(monog,mode='counts')>1)
```
