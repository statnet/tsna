...
As an example, consider the included `moodyContactSim` example object. We will first plot it as a time-aggregated network, ignoring the temporal info and showing all of the edges that are ever active.
...
In situations when we are considering transmission processes on a dynamic network, the aggregate network is not a good representation because it exaggerates the connectivity in the network by ignoring the importance of the order of edge events.  For example, the apparent 8 to 11 to 1 path appears to be the shortest path traverseable in the static aggregate network.  But when we consider the constraints of edge activity, the edge between 1 and 11 becomes inactive long before the edge between 8 and 11 activates, there is no suchpath in the dynamic network. 
...
Most of these other path types would be useful for certain applications, but we do not yet have algorithms implemented for effiently calculating them on non-trivial networks.  

Again, it is important to rember that the *earliest* temporal path is not always the same as the *shortest* (fewest step) temporal path. As in this example below
```{r}
par(mfcol=c(1,2))
plotPaths(moodyContactSim,tPath(moodyContactSim,v=10),coord=coords, main='earliest fwd path from v10')
plotPaths(moodyContactSim,tPath(moodyContactSim,v=10,type = 'fewest.steps'),coord=coords, main='fewest.step fwd path from v10')
par(mfcol=c(1,2))
```
```{r}

plotPaths(moodyContactSim,v10path,coord=coords, main='fwd path from v10')
shortEarly<-network.initialize(6)
add.edges.active(shortEarly,tail=1,head=2,onset=1, terminus=10)
add.edges.active(shortEarly,tail=2,head=3,onset=2, terminus=10)
add.edges.active(shortEarly,tail=3,head=4,onset=3, terminus=10)
add.edges.active(shortEarly,tail=4,head=5,onset=4, terminus=10)
add.edges.active(shortEarly,tail=1,head=6,onset=5, terminus=10)
add.edges.active(shortEarly,tail=6,head=4,onset=1, terminus=10)

plot(shortEarly,displaylabels=TRUE,edge.label=get.edge.activity(shortEarly))

 earliest<-tPath(pathExample,v=1)
 shortest<-tPath(pathExample,v=1,type = 'fewest.steps')
plotPaths(shortEarly,list(shortest,earliest))
...
Notice the spikes at the beginning and ends of the network caused by the truncation (censoring) of edges by the network's observation time window.
...
in dynamic networks". RR-4589, 2002.
 https://hal.inria.fr/inria-00071996/document
