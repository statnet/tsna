...
This package provides tools for working with dynamic networks (or processes on networks) in which the ordering and timing of ties is important.  The tools are need for networks having densities and edge-turnover rates in a parameter space such that the time-collapsed aggregate network give too great a distoration of the connectivity for useful analsysis. 

Although the `networkDynamic` package is designed to support both continuous- and discrete-time models of networks, the initial version of `tsna` is primairly focused on working with discrete time networks of the sort that might be generated from simulation. 

As always, a general help file for the package can be displayed, and the individual arguments for each function are documented in more detail in the function's help page. 
...
The data structure provided by `networkDynamic` objects assumes that the vertices and (directed or non-directed) edges of a network have multiple 'activity spells' associated with them which indicate when they are 'active' or exist from the perspective of the model.  Each spell is an interval with an onset and terminus time. Each edge or vertex an turn on and off multiple times during the period over which the network is observed. 

As an example, consider the incldued `moodyContactSim` example object. 
...
We can view the acivity spells associated with the network's edges
...
Notice that if we view the network that is actually active at any specific time point, it is much less connected that it appears in the time-collapsed network.  In fact, at most time points there is only a single active edge. 

```{r}
plot(network.extract(moodyContactSim,at=50),displaylabels = TRUE,main='network at time 50')
plot(network.extract(moodyContactSim,at=700),displaylabels = TRUE,main='network at time 700')
```
...
`edgeDuration` `tDegree` `tEdgeDensity` `connectedDuration`
...
The durations functions also have the ability to count the number of spells instead of aggregating their durations. This is mostly useful for working with event-based continuous data (see below) but also makes it so we can quickly asses if the network has re-occuring ties. In other words, which edges have more than one activity spell associated with them?
```{r}
which(edgeDuration(monog,mode='counts')>1)
which(edgeDuration(moodyContactSim,mode='counts')>1)
```

All of the edges in the `moodyContactSim` network are only active once (making many times of calculations much simpler), where in the `monog` network, there are a few edges that toggle on and off and then back on again. 
...
These functions are indented to provide descriptive stats about how quickly the elements of the network are changing relative to the time units. 
...
Also, the forward reachable set is not the same as the backwards reachable set
